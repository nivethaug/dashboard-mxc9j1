...
def restart_pm2(project_name: str = None):
    ...
    Args:
        project_name: Project name (uses dashboard-mxc9j1 placeholder by default)
    """
    print("\n" + "="*50)
    print("PM2 RESTART")
    print("="*50)
    ...
    if not project_name:
        project_name = "dashboard-mxc9j1"
    
    return run(f"sudo pm2 restart dashboard-mxc9j1-frontend")
...
def main():
    ...
    if not args.no_restart and success:
        restart_pm2()  # Uses dashboard-mxc9j1 placeholder
        reload_nginx()
